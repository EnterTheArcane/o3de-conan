...
class Recipe(ConanFile):
    ...
    def validate(self):
        if (
            ...
            and Version(self.version) < "3.4.0"
        ):
            raise ConanInvalidConfiguration(
                f"{self.ref} does not support Apple ARM CPUs"
            )
    ...
    def build_requirements(self):
        ...
        if self.settings_build.os == "Windows" and self.settings.get_safe(
            "compiler.runtime"
        ):
    ...
    def generate(self):
        ...
        if self.settings_build.os == "Windows" and self.settings.get_safe(
            ...
            compile_wrapper = unix_path(
                self, os.path.join(self.source_folder, "msvcc.sh")
            )
            ...
            env.define(
                "LIBTOOL",
                unix_path(self, os.path.join(self.source_folder, "ltmain.sh")),
            )
            env.define(
                "INSTALL",
                unix_path(self, os.path.join(self.source_folder, "install-sh")),
            )
        tc.generate(env=env)
