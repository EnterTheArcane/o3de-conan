...
class XZUtilsConan(ConanFile):
    ...
    def _effective_msbuild_type(self):
        ...
        return "{}{}".format(
            ...
            (
                "MT"
                if is_msvc_static_runtime(self) and self.settings.build_type != "Debug"
                else ""
            ),
        )
    ...
    def build_requirements(self):
        if (
            ...
            and not self._use_msbuild
            and (Version(self.version) < "5.8.1" or self.settings.os == "Android")
        ):
            self.win_bash = True
    ...
    def _msvc_sln_folder(self):
        if (
            ...
            and Version(self.settings.compiler) >= "191"
        ):
            return "vs2017"
        return "vs2013"
    ...
    def _build_msvc(self):
        build_script_folder = os.path.join(
            self.source_folder, "windows", self._msvc_sln_folder
        )
        ...
        msbuild.build(
            os.path.join(build_script_folder, solution_file),
            targets=[self._msbuild_target],
        )
    ...
    def build(self):
        ...
        if self._use_msbuild:
            ...
            cmake = CMake(self)
            rmdir(
                self, os.path.join(self.source_folder, "tests")
            )  # optionally included
            cmake.configure()
            cmake.build()
        else:
