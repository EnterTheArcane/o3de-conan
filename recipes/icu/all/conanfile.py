...
class ICUConan(ConanFile):
        ...
        if self.options.dat_package_file:
            if not os.path.exists(str(self.options.dat_package_file)):
                raise ConanInvalidConfiguration(
                    "Non-existent dat_package_file specified"
                )
    ...
    def generate(self):
        ...
        if (
            Version(self.version) >= "75.1"
            and not self.settings.compiler.cppstd
            and is_msvc(self)
        ):
        ...
        if self.settings.os != "Windows":
            ...
            tc.configure_args.append(
                f"--with-data-packaging={self.options.data_packaging}"
            )
        tc.generate()
    ...
    def build(self):
        ...
        autotools.configure(
            build_script_folder=os.path.join(self.source_folder, "source")
        )
        autotools.make()
    ...
    def _data_filename(self):
        ...
        suffix = (
            "b"
            if arch
            in {"ppc32", "ppc64", "sparc", "sparcv9", "s390", "s390x", "mips", "mips64"}
            else "l"
        )
    ...
    def _data_path(self):
        ...
        data_dir = os.path.join(
            self.package_folder, "lib", data_dir_name, str(self.version)
        )
    ...
    def package_info(self):
        ...
        prefix = (
            "s" if self.settings.os == "Windows" and not self.options.shared else ""
        )
        suffix = (
            "d"
            if self.settings.os == "Windows" and self.settings.build_type == "Debug"
        ...
        # icudata
        self.cpp_info.components["icu-data"].set_property(
            "cmake_target_name", "ICU::data"
        )
        icudata_libname = "icudt" if self.settings.os == "Windows" else "icudata"
        self.cpp_info.components["icu-data"].libs = [
            f"{prefix}{icudata_libname}{suffix}"
        ]
        if not self.options.shared:
        ...
        self.cpp_info.components["icu-data-alias"].set_property(
            "cmake_target_name", "ICU::dt"
        )
        ...
        # icui18n
        self.cpp_info.components["icu-i18n"].set_property(
            "cmake_target_name", "ICU::i18n"
        )
        ...
        self.cpp_info.components["icu-i18n"].libs = [
            f"{prefix}{icui18n_libname}{suffix}"
        ]
        ...
        self.cpp_info.components["icu-i18n-alias"].set_property(
            "cmake_target_name", "ICU::in"
        )
        self.cpp_info.components["icu-i18n-alias"].requires = ["icu-i18n"]

        # icuio
        if self.options.with_icuio:
            self.cpp_info.components["icu-io"].set_property(
                "cmake_target_name", "ICU::io"
            )
        ...
        if self.settings.os != "Windows" and self.options.data_packaging in [
            ...
            data_path = os.path.join(
                self.package_folder, "res", self._data_filename
            ).replace("\\", "/")
        ...
        if self._enable_icu_tools:
            # icutu
            self.cpp_info.components["icu-tu"].set_property(
                "cmake_target_name", "ICU::tu"
            )
            ...
            # icutest
            self.cpp_info.components["icu-test"].set_property(
                "cmake_target_name", "ICU::test"
            )
