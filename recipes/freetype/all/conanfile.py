...
class FreetypeConan(ConanFile):
    ...
    def _make_freetype_config(self, version):
        ...
        replace_in_file(
            self, freetype_config, r"%LIBSSTATIC_CONFIG%", r"$conan_staticlibs"
        )
        replace_in_file(
            self,
            freetype_config,
    ...
    def _extract_libtool_version(self):
        conf_raw = load(
            self, os.path.join(self.source_folder, "builds", "unix", "configure.raw")
        )
        return (
            re.search(r"^version_info='([0-9:]+)'", conf_raw, flags=re.M)
            .group(1)
            .replace(":", ".")
        )
    ...
    def package(self):
        ...
        if self.settings.os == "Windows" and not self.options.shared:
            rename(
                self,
                os.path.join(self.package_folder, "lib", "libfreetype.a"),
                os.path.join(self.package_folder, "lib", "freetype.lib"),
            )
        ...
        if (
            ...
            dst = os.path.join(
                self.package_folder, "bin", f"{prefix}freetype{suffix}.dll"
            )
