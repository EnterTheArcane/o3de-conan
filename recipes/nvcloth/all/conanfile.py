...
class NvclothConan(ConanFile):
    ...
    def validate(self):
        ...
        if self.settings.os in ["Windows", "Macos"] and not self.options.shared:
            raise ConanInvalidConfiguration(
                f"Static builds are not supported on {self.settings.os}"
            )
        if self.settings.os in ["iOS", "Android"] and self.options.shared:
            raise ConanInvalidConfiguration(
                f"Shared builds are not supported on {self.settings.os}"
            )

        if self.settings.build_type not in ["Debug", "RelWithDebInfo", "Release"]:
            raise ConanInvalidConfiguration(
                f"{self.settings.build_type} build_type is not supported"
            )
    ...
    def _patch_sources(self):
        ...
        shutil.copy(
            os.path.join(
                self.source_folder, "NvCloth", "include", "NvCloth", "Callbacks.h"
            ),
            os.path.join(
                self.source_folder,
                "NvCloth",
    ...
    def package(self):
        ...
        copy(
            ...
            src=os.path.join(self.source_folder, "NvCloth", "include"),
        )
        copy(
            self,
            "*.h",
            dst=os.path.join(self.package_folder, "include"),
            src=os.path.join(self.source_folder, "NvCloth", "extensions", "include"),
        )
        copy(
            self,
            "*.h",
            dst=os.path.join(self.package_folder, "include"),
            src=os.path.join(self.source_folder, "PxShared", "include"),
        )
        copy(
            self,
            "*.a",
