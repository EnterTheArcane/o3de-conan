...
class DracoConan(ConanFile):
    ...
    def generate(self):
        ...
        if Version(self.version) < "1.4.0":
            tc.variables["ENABLE_POINT_CLOUD_COMPRESSION"] = (
                self.options.enable_point_cloud_compression
            )
            tc.variables["ENABLE_MESH_COMPRESSION"] = (
                self.options.enable_mesh_compression
            )
            if self.options.enable_mesh_compression:
                tc.variables["ENABLE_STANDARD_EDGEBREAKER"] = (
                    self.options.enable_standard_edgebreaker
                )
                tc.variables["ENABLE_PREDICTIVE_EDGEBREAKER"] = (
                    self.options.enable_predictive_edgebreaker
                )
            tc.variables["ENABLE_BACKWARDS_COMPATIBILITY"] = (
            ...
            tc.variables["DRACO_POINT_CLOUD_COMPRESSION"] = (
                self.options.enable_point_cloud_compression
            )
            tc.variables["DRACO_MESH_COMPRESSION"] = (
                self.options.enable_mesh_compression
            )
            if self.options.enable_mesh_compression:
                tc.variables["DRACO_STANDARD_EDGEBREAKER"] = (
                    self.options.enable_standard_edgebreaker
                )
                tc.variables["DRACO_PREDICTIVE_EDGEBREAKER"] = (
                    self.options.enable_predictive_edgebreaker
                )
            tc.variables["DRACO_ANIMATION_ENCODING"] = False
            tc.variables["DRACO_BACKWARDS_COMPATIBILITY"] = (
                self.options.enable_backwards_compatibility
            )
