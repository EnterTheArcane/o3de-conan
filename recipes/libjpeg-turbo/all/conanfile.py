...
class LibjpegTurboConan(ConanFile):
    ...
    def configure(self):
        ...
        if (
            self.options.get_safe("enable12bit")
    ...
    def validate(self):
        if self.options.get_safe("enable12bit") and (
        ...
        if self.options.get_safe("java") and not self.options.shared:
            raise ConanInvalidConfiguration(
                "java wrapper requires shared libjpeg-turbo"
            )
    ...
    def _is_arithmetic_encoding_enabled(self):
        return (
            self.options.get_safe("arithmetic_encoder", False)
            or self.options.libjpeg7_compatibility
            or self.options.libjpeg8_compatibility
        )
    ...
    def _is_arithmetic_decoding_enabled(self):
        return (
            self.options.get_safe("arithmetic_decoder", False)
            or self.options.libjpeg7_compatibility
            or self.options.libjpeg8_compatibility
        )
    ...
    def generate(self):
        ...
        if Version(self.version) < "3.0.0":
            tc.variables["WITH_MEM_SRCDST"] = self.options.get_safe(
                "mem_src_dst", False
            )
    ...
    def package_info(self):
        ...
        self.cpp_info.components["jpeg"].set_property(
            "cmake_module_target_name", "JPEG::JPEG"
        )
        self.cpp_info.components["jpeg"].set_property(
