...
class Recipe(ConanFile):
    ...
    def generate(self):
        save(
            ...
            """[Paths]
Prefix = {}""".format(
                self.dependencies["qt"].package_folder.replace("\\", "/")
            ),
        )
    ...
    def _meson_supported(self):
        return (
            ...
            and not tools.os_info.is_macos
    ...
    def _build_with_meson(self):
        if self._meson_supported():
            ...
            with tools.environment_append(RunEnvironment(self).vars):
                ...
                try:
                    meson.configure(
                        build_folder="meson_folder", defs={"cpp_std": "c++11"}
                    )
                except ConanInvalidConfiguration:
                    self.output.info(
                        open("meson_folder/meson-logs/meson-log.txt", "r").read()
                    )
                    raise
                meson.build()
