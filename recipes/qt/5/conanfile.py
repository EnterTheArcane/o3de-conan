...
class QtConan(ConanFile):
    ...
    def validate_build(self):
        if self.options.qtwebengine:
            ...
            if not python_exe:
                msg = (
                    "Python2 must be available in PATH "
            ...
            cmd_v = (
                f'"{python_exe}" -c "import platform;print(platform.python_version())"'
            )
    ...
    def config_options(self):
        ...
        if self.settings.compiler in ["gcc", "clang"]:
            if Version(self.settings.compiler.version) < "5.0":
                raise ConanInvalidConfiguration(
                    "qt 5.15.X does not support GCC or clang before 5.0"
                )
        if (
    ...
    def configure(self):
        ...
        for s in config.sections():
            ...
            if status not in ("obsolete", "ignore"):
                ...
                submodules_tree[modulename] = {
                    "status": status,
                    "path": str(config.get(section, "path")),
                    "depends": [],
                }
                if config.has_option(section, "depends"):
                    submodules_tree[modulename]["depends"] = [
        ...
        for module in self._submodules:
            if module not in submodules_tree:
                self._debug_output(
                    f"qt5: removing {module} from options as it is not an option for this version, or it is ignored or obsolete"
                )
        ...
        for module in [m for m in self._submodules if m in submodules_tree]:
            ...
            if self.options.get_safe(f"{status}_modules"):
                if not is_disabled:
                    ...
                    self.output.warning(
                        f"qt5: {module} requested because {status}_modules=True"
        ...
        if required_modules:
            self._debug_output(
                f"qt5: required_modules modules {list(required_modules.keys())}"
            )
        ...
        for status in self._module_statuses:
            ...
            self._debug_output(
                f"qt5 removing convenience option: {option_name},"
    ...
    def validate(self):
        ...
        if self.settings.os == "Android":
            ...
            if not self.options.get_safe("android_sdk", ""):
                raise ConanInvalidConfiguration(
                    "Path to Android SDK is required to build Qt"
                )
        ...
        if (
            not self.options.with_doubleconversion
            and self.settings.get_safe("compiler.libcxx") != "libc++"
        ):
            raise ConanInvalidConfiguration(
    ...
    def source(self):
        ...
        copy(
            ...
            "*",
            os.path.join(
                self.source_folder, "qt5", "qtbase", "src", "3rdparty", "angle"
            ),
            self.angle_path,
        )
    ...
    def generate(self):
        ...
        if self.settings.os == "Windows":
            env.prepend_path(
                "PATH", os.path.join(self.source_folder, "qt5", "gnuwin32", "bin")
            )
    ...
    def build(self):
        ...
        args.append(
            "--fontconfig="
            + ("yes" if self.options.get_safe("with_fontconfig", False) else "no")
        )
        args.append(
            "--icu=" + ("yes" if self.options.get_safe("with_icu", False) else "no")
        )
        args.append(
            "--sql-mysql="
            + ("yes" if self.options.get_safe("with_mysql", False) else "no")
        )
        ...
        if self.options.qtmultimedia:
            args.append(
                "--alsa="
                + ("yes" if self.options.get_safe("with_libalsa", False) else "no")
            )
            args.append(
                "--gstreamer"
                if self.options.get_safe("with_gstreamer", False)
        ...
        for dependency in self.dependencies.direct_host.values():
            args += [
                f'-I "{s}"'
                for s in dependency.cpp_info.aggregated_components().includedirs
            ]
            args += [
                f"-D {s}" for s in dependency.cpp_info.aggregated_components().defines
            ]
        ...
        if "libmysqlclient" in [
            d.ref.name for d in self.dependencies.direct_host.values()
        ]:
            args.append(
                '-mysql_config "%s"'
                % os.path.join(
                    self.dependencies["libmysqlclient"].package_folder,
                    "bin",
                    "mysql_config",
                )
            )
        if "mariadb-connector-c" in [
            d.ref.name for d in self.dependencies.direct_host.values()
        ]:
            args.append(
                '-mysql_config "%s"'
                % os.path.join(
                    self.dependencies["mariadb-connector-c"].package_folder,
                    "bin",
                    "mysql_config",
                )
            )
        if "libpq" in [d.ref.name for d in self.dependencies.direct_host.values()]:
            args.append(
                '-psql_config "%s"'
                % os.path.join(
                    self.dependencies["libpq"].package_folder, "bin", "pg_config"
                )
            )
        if self.settings.os == "Macos":
            args += ["-no-framework"]
        ...
        if self._settings_build.os == "Macos":
            ...
            if not cross_building(self):
                for libpath in (
                    VirtualRunEnv(self).vars().get("DYLD_LIBRARY_PATH", "").split(":")
                ):
        ...
        if self.settings.compiler == "apple-clang" and self.options.qtmultimedia:
            ...
            args += [
                'QMAKE_CXXFLAGS+="-D_LIBCPP_ENABLE_CXX17_REMOVED_UNARY_BINARY_FUNCTION=1"'
            ]
        ...
        with chdir(self, "build_folder"):
            ...
            self.run(
                "%s %s"
                % (os.path.join(self.source_folder, "qt5", "configure"), " ".join(args))
            )
    ...
    def package(self):
        ...
        save(
            self,
            os.path.join(self.package_folder, "bin", "qt.conf"),
            """[Paths]
Prefix = ..""",
        )
        copy(
        ...
        for m in os.listdir(os.path.join(self.package_folder, "lib", "cmake")):
            module = os.path.join(
                self.package_folder, "lib", "cmake", m, f"{m}Macros.cmake"
            )
        ...
        save(
            self,
            os.path.join(self.package_folder, self._cmake_core_extras_file),
            filecontents,
        )
        ...
        def _create_private_module(module, dependencies=[]):
            ...
            dependencies_string = ";".join(
                f"Qt5::{dependency}" for dependency in dependencies
            )
            ...
            save(
                self,
                os.path.join(self.package_folder, self._cmake_qt5_private_file(module)),
                contents,
            )
    ...
    def package_info(self):
        ...
        def _create_module(module, requires=[], has_include_dir=True):
            ...
            self.cpp_info.components[componentname].set_property(
                "cmake_target_name", f"Qt5::{module}"
            )
            self.cpp_info.components[componentname].set_property(
                "cmake_target_aliases", [f"Qt::{module}"]
            )
            self.cpp_info.components[componentname].set_property(
                "pkg_config_name", f"Qt5{module}"
            )
        ...
        def _create_plugin(pluginname, libname, plugintype, requires):
            ...
            self.cpp_info.components[componentname].set_property(
                "cmake_target_name", f"Qt5::{pluginname}"
            )
            self.cpp_info.components[componentname].set_property(
        ...
        if self.options.gui:
            ...
            if self.settings.os == "Windows":
                self.cpp_info.components["qtFontDatabaseSupport"].system_libs.extend(
                    ["advapi32", "ole32", "user32", "gdi32"]
                )
            elif is_apple_os(self):
                self.cpp_info.components["qtFontDatabaseSupport"].frameworks.extend(
            ...
            if self.options.get_safe("with_fontconfig"):
                self.cpp_info.components["qtFontDatabaseSupport"].requires.append(
                    "fontconfig::fontconfig"
                )
            ...
            if is_apple_os(self):
                ...
                if self.settings.os == "Macos":
                    self.cpp_info.components["qtClipboardSupport"].frameworks.append(
                        "AppKit"
                    )
            ...
            if self.settings.os == "Windows":
                ...
                _create_plugin(
                    "QWindowsIntegrationPlugin", "qwindows", "platforms", windows_reqs
                )
                _create_plugin(
                ...
                _create_plugin(
                    "QCocoaIntegrationPlugin", "qcocoa", "platforms", cocoa_reqs
                )
                ...
                _create_plugin(
                    ...
                    "qios",
                    "platforms",
                    ["ClipboardSupport", "FontDatabaseSupport", "GraphicsSupport"],
                )
                ...
                _create_plugin(
                    ...
                    "qminimal",
                    "platforms",
                    ["EventDispatcherSupport", "FontDatabaseSupport"],
                )
                ...
                if self.options.get_safe("with_x11", False):
                    _create_module(
                        "XkbCommonSupport",
                        ["Core", "Gui", "xkbcommon::libxkbcommon-x11"],
                    )
                    xcb_qpa_reqs = [
                        "Core",
                        "Gui",
                    ...
                    _create_module(
                        "XkbCommonSupport", ["Core", "Gui", "xkbcommon::libxkbcommon"]
                    )
        ...
        if self.options.with_odbc:
            if self.settings.os != "Windows":
                _create_plugin(
                    "QODBCDriverPlugin", "qsqlodbc", "sqldrivers", ["odbc::odbc"]
                )
        networkReqs = []
        if self.options.openssl:
        ...
        if self.options.qtdeclarative:
            ...
            self.cpp_info.components["qtQmlImportScanner"].set_property(
                "cmake_target_name", "Qt5::QmlImportScanner"
            )
            self.cpp_info.components["qtQmlImportScanner"].set_property(
                "cmake_target_aliases", ["Qt::QmlImportScanner"]
            )
            self.cpp_info.components["qtQmlImportScanner"].names[
                "cmake_find_package"
            ] = "QmlImportScanner"  # this is an alias for Qml and there to integrate with existing consumers
            self.cpp_info.components["qtQmlImportScanner"].names[
                "cmake_find_package_multi"
            ] = "QmlImportScanner"
        ...
        if self.options.qttools and self.options.gui and self.options.widgets:
            self.cpp_info.components["qtLinguistTools"].set_property(
                "cmake_target_name", "Qt5::LinguistTools"
            )
            self.cpp_info.components["qtLinguistTools"].set_property(
        ...
        if self.options.qtquick3d and self.options.gui:
            ...
            _create_module(
                "Quick3DAssetImport", ["Gui", "Qml", "Quick3DRender", "Quick3DUtils"]
            )
            _create_module(
                "Quick3DRuntimeRender",
                ["Quick3DRender", "Quick3DAssetImport", "Quick3DUtils"],
            )
        ...
        if self.options.qtlocation:
            ...
            _create_plugin(
                "GeoServiceProviderFactoryEsri", "qtgeoservices_esri", "geoservices", []
            )
            _create_plugin(
            ...
            _create_plugin(
                "QGeoServiceProviderFactoryOsm", "qtgeoservices_osm", "geoservices", []
            )
            _create_plugin(
        ...
        if self.options.qtgamepad:
            ...
            if self.settings.os == "Linux":
                _create_plugin(
                    "QEvdevGamepadBackendPlugin", "evdevgamepad", "gamepads", []
                )
            if self.settings.os == "Macos":
                _create_plugin(
                    "QDarwinGamepadBackendPlugin", "darwingamepad", "gamepads", []
                )
            if self.settings.os == "Windows":
                _create_plugin(
                    "QXInputGamepadBackendPlugin", "xinputgamepad", "gamepads", []
                )
        ...
        if self.options.qt3d:
            ...
            _create_plugin(
                "GLTFGeometryLoaderPlugin", "gltfgeometryloader", "geometryloaders", []
            )
            _create_plugin(
                "GLTFSceneExportPlugin", "gltfsceneexport", "sceneparsers", []
            )
            _create_plugin(
                "GLTFSceneImportPlugin", "gltfsceneimport", "sceneparsers", []
            )
            ...
            _create_module(
                "3DInput",
                ["3DCore", "Gui"] + (["Gamepad"] if self.options.qtgamepad else []),
            )
            ...
            _create_module(
                "3DQuickRender", ["3DRender", "3DQuick", "3DCore", "Gui", "Qml"]
            )
            _create_module(
                "3DQuickScene2D", ["3DRender", "3DQuick", "3DCore", "Gui", "Qml"]
            )
        ...
        if self.options.qtmultimedia:
            ...
            _create_plugin(
                "QM3uPlaylistPlugin", "qtmultimedia_m3u", "playlistformats", []
            )
            if self.options.with_gstreamer:
                ...
                _create_plugin(
                    "QGstreamerAudioDecoderServicePlugin",
                    "gstaudiodecoder",
                    "mediaservice",
                    [],
                )
            ...
            if self.settings.os == "Linux":
                if self.options.with_gstreamer:
                    _create_plugin(
                        "CameraBinServicePlugin", "gstcamerabin", "mediaservice", []
                    )
                if self.options.get_safe("with_libalsa", False):
            ...
            if self.settings.os == "Macos":
                ...
                _create_plugin(
                    "AVFMediaPlayerServicePlugin", "qavfmediaplayer", "mediaservice", []
                )
        ...
        if self.options.get_safe("qtactiveqt"):
            ...
            self.cpp_info.components["qtAxBase"].includedirs = [
                "include",
                os.path.join("include", "ActiveQt"),
            ]
            self.cpp_info.components["qtAxBase"].system_libs.extend(
                ["ole32", "oleaut32", "user32", "gdi32", "advapi32"]
            )
            if self.settings.compiler == "gcc":
                self.cpp_info.components["qtAxBase"].system_libs.append("uuid")
            _create_module("AxContainer", ["Core", "Gui", "Widgets", "AxBase"])
            self.cpp_info.components["qtAxContainer"].includedirs = [
                os.path.join("include", "ActiveQt")
            ]
            _create_module("AxServer", ["Core", "Gui", "Widgets", "AxBase"])
            self.cpp_info.components["qtAxServer"].includedirs = [
                os.path.join("include", "ActiveQt")
            ]
        ...
        if not self.options.shared:
            if self.settings.os == "Windows":
                ...
                if self.options.get_safe("qtwinextras"):
                    self.cpp_info.components["qtWinExtras"].system_libs.append(
                        "dwmapi"
                    )  # qtwinextras requires "DwmGetColorizationColor" which is in "dwmapi.lib" library

            if is_apple_os(self):
                self.cpp_info.components["qtCore"].frameworks.append(
                    "CoreServices"
                    if self.settings.os == "Macos"
                    else "MobileCoreServices"
                )
        ...
        qt5core_config_extras_mkspec_dir_cmake = load(
            self,
            os.path.join(
        ...
        mkspecs_dir_end = qt5core_config_extras_mkspec_dir_cmake.find(
            '"', mkspecs_dir_begin
        )
        mkspecs_path = qt5core_config_extras_mkspec_dir_cmake[
            mkspecs_dir_begin:mkspecs_dir_end
        ]
        ...
        for object_dir in objects_dirs:
            for m in os.listdir(object_dir):
                ...
                for sub_dir in os.listdir(submodules_dir):
                    ...
                    obj_files = [
                        os.path.join(submodule_dir, file)
                        for file in os.listdir(submodule_dir)
                    ]
    ...
    def _gather_libs(self, p):
        libs = [
            "-l" + i
            for i in p.cpp_info.aggregated_components().libs
            + p.cpp_info.aggregated_components().system_libs
        ]
        if is_apple_os(self):
            libs += [
                "-framework " + i for i in p.cpp_info.aggregated_components().frameworks
            ]
