...
Prefix = {path}""",
        ...
        if "qt" in self.dependencies.build:
            ...
            for tool in ["moc", "rcc", "uic"]:
                tc.cache_variables[f"CMAKE_AUTO{tool.upper()}_EXECUTABLE"] = (
                    os.path.join(
                        qt_tools_rootdir,
                        f"{tool}.exe" if self.settings_build.os == "Windows" else tool,
                    )
                )
        tc.generate()
    ...
    def test(self):
        if can_run(self):
            ...
            if self.settings.os == "Macos":
                bin_macos_path = os.path.join(
                    self.cpp.build.bindirs[0],
                    "test_macos_bundle.app",
                    "Contents",
                    "MacOS",
                    "test_macos_bundle",
                )
        ...
        qt_tools_dir = self.dependencies.host["qt"].conf_info.get(
            "user.qt:tools_directory"
        )
        assert os.path.isdir(qt_tools_dir)
        moc = os.path.join(
            qt_tools_dir, "moc.exe" if self.settings.os == "Windows" else "moc"
        )
