...
class QtConan(ConanFile):
            ...
            if status not in ["obsolete", "ignore", "additionalLibrary"]:
                if status not in self._module_statuses:
                    raise ConanException(
                        f"module {modulename} has status {status} which is not in self._module_statuses {self._module_statuses}"
                    )
                assert (
                    modulename in self._submodules
                ), f"module {modulename} not in self._submodules"
                ...
                if config.has_option(section, "depends"):
                    self._submodules_tree[modulename]["depends"] = [
    ...
    def config_options(self):
        ...
        for submodule in self._submodules:
            if submodule not in self._get_module_tree:
                self.output.debug(
                    f"Qt6: Removing {submodule} option as it is not in the module tree for this version, or is marked as obsolete or ignore"
                )
    ...
    def configure(self):
        ...
        requested_modules = set(
            [module for module in self._submodules if self.options.get_safe(module)]
        )
        ...
        required_but_disabled = [
            m for m in required_modules.keys() if self.options.get_safe(m) == False
        ]
        if required_modules:
            self.output.debug(
                f"qt6: required_modules modules {list(required_modules.keys())}"
            )
        ...
        self.no_copy_source = not (
            self.settings_build.os == "Windows" and self.options.get_safe("qtwebengine")
        )
    ...
    def validate(self):
        ...
        if (
            self.settings.compiler == "apple-clang"
            and Version(self.settings.compiler.version) < "13"
        ):
            # note: assuming that by now, any xcode 13 is updated to the latest 13.4.1
            raise ConanInvalidConfiguration(
                "apple-clang >= 13.1 is required by qt >= 6.6.1 cf QTBUG-119490"
            )
        ...
        if (
            ...
            raise ConanInvalidConfiguration(
                "Visual Studio 2022 (MSVC 1930 or newer) is required by qt >= 6.8.3"
            )
        ...
        if (
            ...
            raise ConanInvalidConfiguration(
                "apple-clang >= 14 is required by qt >= 6.8.3"
            )
        ...
        if (
            ...
            and self.options.get_safe("opengl", "no") == "desktop"
        ):
            raise ConanInvalidConfiguration(
                "OpenGL desktop is not supported on Android."
            )

        if (
            self.settings.os != "Windows"
            and self.options.get_safe("opengl", "no") == "dynamic"
        ):
            raise ConanInvalidConfiguration(
        ...
        ) and not self.options.get_safe("with_freetype", False):
            raise ConanInvalidConfiguration(
        ...
        if not self.options.with_pcre2:
            raise ConanInvalidConfiguration(
                "pcre2 is actually required by qt (QTBUG-92454). please use option qt:with_pcre2=True"
            )
        ...
        if self.options.get_safe("qtspeech") and not self.options.qtdeclarative:
            raise ConanInvalidConfiguration(
                "qtspeech requires qtdeclarative, cf QTBUG-108381"
            )
    ...
    def generate(self):
        ...
        tc.set_property(
            "wayland::wayland-client", "cmake_target_name", "Wayland::Client"
        )
        tc.set_property(
            "wayland::wayland-server", "cmake_target_name", "Wayland::Server"
        )
        tc.set_property(
            "wayland::wayland-cursor", "cmake_target_name", "Wayland::Cursor"
        )
        ...
        tc.variables["FEATURE_optimize_size"] = (
            "ON" if self.settings.get_safe("build_type") == "MinSizeRel" else "OFF"
        )

        for module in self._get_module_tree:
            tc.variables[f"BUILD_{module}"] = (
                "ON" if getattr(self.options, module) else "OFF"
            )
        ...
        if self.options.cross_compile:
            tc.variables["QT_QMAKE_DEVICE_OPTIONS"] = (
                f"CROSS_COMPILE={self.options.cross_compile}"
            )
        if cross_building(self):
            # Mainly to locate Qt6HostInfoConfig.cmake
            tc.cache_variables["QT_HOST_PATH"] = self.dependencies.direct_build[
                "qt"
            ].package_folder
            # Stand-in for Qt6CoreTools - which is loaded for the executable targets
            tc.cache_variables["CMAKE_PROJECT_Qt_INCLUDE"] = os.path.join(
                self.dependencies.direct_build["qt"].package_folder,
                self._cmake_executables_file,
            )
        ...
        current_cpp_std = self.settings.get_safe(
            "compiler.cppstd", default_cppstd(self)
        )
    ...
    def source(self):
        ...
        replace_in_file(
            self,
            os.path.join(
                self.source_folder, "qtbase", "cmake", "FindWrapSystemZLIB.cmake"
            ),
            '"-lz"',
            "ZLIB::ZLIB",
        )
        ...
        replace_in_file(
            self,
            os.path.join(
                self.source_folder, "qtbase", "cmake", "QtAutoDetectHelpers.cmake"
            ),
        ...
        replace_in_file(
            self,
            os.path.join(
                self.source_folder, "qtbase", "cmake", "FindWrapVulkanHeaders.cmake"
            ),
            "if(APPLE)",
            "if(APPLE)\n"
    ...
    def _xplatform(self):
        if self.settings.os == "Linux":
            if self.settings.compiler == "gcc":
                ...
                }.get(str(self.settings.arch), "linux-g++")
            ...
            return {"clang": "freebsd-clang", "gcc": "freebsd-g++"}.get(
            ...
            if self.settings.compiler == "sun-cc":
                if self.settings.arch == "sparc":
                    return (
                        "solaris-cc-stlport"
                        if self.settings.compiler.libcxx == "libstlport"
                        else "solaris-cc"
                    )
                if self.settings.arch == "sparcv9":
                    return (
                        "solaris-cc64-stlport"
                        if self.settings.compiler.libcxx == "libstlport"
                        else "solaris-cc64"
                    )
    ...
    def _cmake_executables_file(self):
        return os.path.join(
            "lib", "cmake", "Qt6Core", "conan_qt_executables_variables.cmake"
        )
    ...
    def _cmake_platform_target_setup_file(self):
        return os.path.join(
            "lib", "cmake", "Qt6", "conan_qt_platform_target_setup.cmake"
        )
    ...
    def _cmake_qt6_private_file(self, module):
        return os.path.join(
            ...
            "cmake",
            f"Qt6{module}",
            f"conan_qt_qt6_{module.lower()}private.cmake",
        )
    ...
    def package(self):
        ...
        for mask in ["Find*.cmake", "*Config.cmake", "*-config.cmake"]:
            rm(
                ...
                self.package_folder,
                recursive=True,
                excludes="Qt6HostInfoConfig.cmake",
            )
        ...
        os.remove(
            os.path.join(
                self.package_folder,
                "libexec" if self.settings.os != "Windows" else "bin",
        ...
        for m in os.listdir(os.path.join(self.package_folder, "lib", "cmake")):
            if os.path.isfile(
                os.path.join(self.package_folder, "lib", "cmake", m, f"{m}Macros.cmake")
            ):
                continue
            if glob.glob(
                os.path.join(
                    self.package_folder, "lib", "cmake", m, "QtPublic*Helpers.cmake"
                )
            ):
                continue
            if glob.glob(
                os.path.join(
                    self.package_folder, "lib", "cmake", m, "Qt6QmlPublic*Helpers.cmake"
                )
            ):
                continue
            if m.endswith("Tools"):
                if os.path.isfile(
                    os.path.join(
                        self.package_folder, "lib", "cmake", m, f"{m[:-5]}Macros.cmake"
                    )
                ):
                    continue
        ...
        if self.options.qttools:
            ...
            if (
                (
                    not any(
                        item in disabled_features
                        for item in ["assistant", "toolbutton", "pushbutton"]
                    )
                )
                and self.options.widgets
                and self.options.get_safe("with_libpng")
            ):
        ...
        if self.options.qtdeclarative:
            targets.extend(
                ["qmltyperegistrar", "qmlcachegen", "qmllint", "qmlimportscanner"]
            )
        ...
        for target in targets:
            ...
            for path_ in [
                ...
                assert (
                    False
                ), f"Could not find executable {target}{extension} in {self.package_folder}"
            if not exe_path:
        ...
        save(
            self,
            os.path.join(self.package_folder, self._cmake_executables_file),
            filecontents,
        )

        def _create_private_module(module, dependencies):
            dependencies_string = ";".join(
                f"Qt6::{dependency}" for dependency in dependencies
            )
        ...
        if self.options.qtdeclarative:
            ...
            if self.options.gui and self.options.qtshadertools:
                _create_private_module(
                    "Quick", ["CorePrivate", "GuiPrivate", "QmlPrivate", "Quick"]
                )
        ...
        if self.settings.os in ["Windows", "iOS"]:
            ...
            save(
                self,
                os.path.join(self.package_folder, self._cmake_entry_point_file),
                contents,
            )
        ...
        if self.settings.os == "Windows" or is_msvc(self):
            ...
            save(
                self,
                os.path.join(
                    self.package_folder, self._cmake_platform_target_setup_file
                ),
                contents,
            )
    ...
    def package_info(self):
        ...
        def _get_corrected_reqs(requires):
            ...
            for r in requires:
                if "::" in r:
                    ...
                    assert (
                        corrected_req in self.cpp_info.components
                    ), f"{corrected_req} required but not yet present in self.cpp_info.components"
        ...
        def _create_module(module, requires, has_include_dir=True):
            ...
            self.cpp_info.components[componentname].set_property(
                "cmake_target_aliases", [f"Qt::{module}"]
            )
            self.cpp_info.components[componentname].set_property(
                "pkg_config_name", f"Qt6{module}"
            )
            ...
            if has_include_dir:
                self.cpp_info.components[componentname].includedirs = [
                    "include",
                    os.path.join("include", f"Qt{module}"),
                ]
            self.cpp_info.components[componentname].defines = [
                f"QT_{module.upper()}_LIB"
            ]
            if module != "Core" and "Core" not in requires:
                requires.append("Core")
            self.cpp_info.components[componentname].requires = _get_corrected_reqs(
                requires
            )
        ...
        def _create_plugin(pluginname, libname, plugintype, requires):
            ...
            self.cpp_info.components[componentname].set_property(
                "cmake_target_aliases", [f"Qt::{pluginname}"]
            )
            if not self.options.shared:
                self.cpp_info.components[componentname].libs = [libname + libsuffix]
            self.cpp_info.components[componentname].libdirs = [
                os.path.join("plugins", plugintype)
            ]
            self.cpp_info.components[componentname].includedirs = []
        ...
        self.cpp_info.components["qtPlatform"].set_property(
            "cmake_target_name", "Qt6::Platform"
        )
        self.cpp_info.components["qtPlatform"].set_property(
            "cmake_target_aliases", ["Qt::Platform"]
        )
        self.cpp_info.components["qtPlatform"].includedirs = [
            os.path.join("mkspecs", self._xplatform())
        ]
        ...
        if self.options.gui:
            ...
            if self.settings.os == "Windows":
                ...
                self.cpp_info.components[
                    "qtQWindowsIntegrationPlugin"
                ].system_libs.append("uiautomationcore")
            elif self.settings.os == "Android":
                _create_plugin(
                ...
                self.cpp_info.components["qtGui"].frameworks = [
                    "CoreFoundation",
                    "CoreGraphics",
                    "CoreText",
                    "Foundation",
                ...
                has_metal = "metal" not in disabled_features and self.settings.os in [
                    "Macos",
                    "iOS",
                    "visionOS",
                ]
                ...
                if self.settings.os in ["iOS", "tvOS"]:
                    ...
                    _create_plugin(
                        "QMinimalIntegrationPlugin", "qminimal", "platforms", []
                    )
            elif self.settings.os == "Emscripten":
                _create_plugin(
                    "QWasmIntegrationPlugin", "qwasm", "platforms", ["Core", "Gui"]
                )
        ...
        if self.options.with_sqlite3:
            _create_plugin(
                "QSQLiteDriverPlugin", "qsqlite", "sqldrivers", ["sqlite3::sqlite3"]
            )
        if self.options.with_pq:
            _create_plugin(
                "QPSQLDriverPlugin", "qsqlpsql", "sqldrivers", ["libpq::libpq"]
            )
        if self.options.with_odbc:
            _create_plugin("QODBCDriverPlugin", "qsqlodbc", "sqldrivers", [])
            if self.settings.os != "Windows":
                self.cpp_info.components["qtQODBCDriverPlugin"].requires.append(
                    "odbc::odbc"
                )
            else:
                self.cpp_info.components["qtQODBCDriverPlugin"].system_libs.append(
                    "odbc32"
                )
        networkReqs = []
        if self.options.openssl:
        ...
        if self.options.qtdeclarative:
            ...
            self.cpp_info.components["qtQmlImportScanner"].set_property(
                "cmake_target_name", "Qt6::QmlImportScanner"
            )
            self.cpp_info.components["qtQmlImportScanner"].set_property(
                "cmake_target_aliases", ["Qt::QmlImportScanner"]
            )
            self.cpp_info.components["qtQmlImportScanner"].requires = (
                _get_corrected_reqs(["Qml"])
            )
            if qt_quick_enabled:
        ...
        if self.options.qttools and self.options.gui and self.options.widgets:
            ...
            self.cpp_info.components["qtLinguistTools"].set_property(
                "cmake_target_aliases", ["Qt::LinguistTools"]
            )
        ...
        if self.options.get_safe("qtvirtualkeyboard") and qt_quick_enabled:
            ...
            _create_plugin(
                ...
                ["Gui", "Qml", "VirtualKeyboard"],
            )
            _create_plugin(
                "QtVirtualKeyboardHangulPlugin",
                "qtvirtualkeyboard_hangul",
                "virtualkeyboard",
                ["Gui", "Qml", "VirtualKeyboard"],
            )
            _create_plugin(
                "QtVirtualKeyboardMyScriptPlugin",
                "qtvirtualkeyboard_myscript",
                "virtualkeyboard",
                ["Gui", "Qml", "VirtualKeyboard"],
            )
            _create_plugin(
                "QtVirtualKeyboardThaiPlugin",
                "qtvirtualkeyboard_thai",
                "virtualkeyboard",
                ["Gui", "Qml", "VirtualKeyboard"],
            )
        if self.options.get_safe("qt3d"):
            ...
            _create_module(
                "3DExtras", ["Gui", "3DCore", "3DInput", "3DLogic", "3DRender"]
            )
            _create_plugin(
                "DefaultGeometryLoaderPlugin",
                "defaultgeometryloader",
                "geometryloaders",
                ["3DCore", "3DRender", "Gui"],
            )
            _create_plugin(
                "fbxGeometryLoaderPlugin",
                "fbxgeometryloader",
                "geometryloaders",
                ["3DCore", "3DRender", "Gui"],
            )
            if qt_quick_enabled:
                ...
                _create_module(
                    "3DQuickInput", ["3DCore", "3DInput", "3DQuick", "Gui", "Qml"]
                )
                _create_module(
                    "3DQuickRender", ["3DCore", "3DQuick", "3DRender", "Gui", "Qml"]
                )
                _create_module(
                    "3DQuickScene2D", ["3DCore", "3DQuick", "3DRender", "Gui", "Qml"]
                )
        ...
        if self.options.get_safe("qtopcua"):
            ...
            _create_plugin(
                "QOpen62541Plugin", "open62541_backend", "opcua", ["Network", "OpcUa"]
            )
            _create_plugin(
                "QUACppPlugin", "uacpp_backend", "opcua", ["Network", "OpcUa"]
            )
        ...
        if self.options.get_safe("qtsensors"):
            ...
            _create_plugin(
                "IIOSensorProxySensorPlugin",
                "qtsensors_iio-sensor-proxy",
                "sensors",
                [],
            )
            if self.settings.os == "Linux":
            ...
            _create_plugin(
                "QtSensorGesturePlugin", "qtsensorgestures_plugin", "sensorgestures", []
            )
            _create_plugin(
        ...
        if self.options.get_safe("qtserialbus"):
            _create_module(
                "SerialBus",
                ["SerialPort"] if self.options.get_safe("qtserialport") else [],
            )
        ...
        if self.settings.os in ["Windows", "iOS"]:
            if self.settings.os == "Windows":
                self.cpp_info.components["qtEntryPointImplementation"].set_property(
                    "cmake_target_name", "Qt6::EntryPointImplementation"
                )
                self.cpp_info.components["qtEntryPointImplementation"].set_property(
                    "cmake_target_aliases", ["Qt::EntryPointImplementation"]
                )
                self.cpp_info.components["qtEntryPointImplementation"].libs = [
                    f"Qt6EntryPoint{libsuffix}"
                ]
                self.cpp_info.components["qtEntryPointImplementation"].system_libs = [
                    "shell32"
                ]

                if self.settings.compiler == "gcc":
                    self.cpp_info.components["qtEntryPointMinGW32"].set_property(
                        "cmake_target_name", "Qt6::EntryPointMinGW32"
                    )
                    self.cpp_info.components["qtEntryPointMinGW32"].set_property(
                        "cmake_target_aliases", ["Qt::EntryPointMinGW32"]
                    )
                    self.cpp_info.components["qtEntryPointMinGW32"].system_libs = [
                        "mingw32"
                    ]
            ...
            self.cpp_info.components["qtEntryPointPrivate"].set_property(
                "cmake_target_aliases", ["Qt::EntryPointPrivate"]
            )
            if self.settings.os == "Windows":
                if self.settings.compiler == "gcc":
                    self.cpp_info.components["qtEntryPointPrivate"].defines.append(
                        "QT_NEEDS_QMAIN"
                    )
        ...
        if not self.options.shared:
            if self.settings.os == "Windows":
                ...
                if Version(self.version) >= "6.10":
                    ...
                    self.cpp_info.components["qtCore"].system_libs.extend(
                        ["icuuc", "icuin"]
                    )
                ...
                self.cpp_info.components["qtPrintSupport"].system_libs.append(
                    "comdlg32"
                )
                self.cpp_info.components["qtPrintSupport"].system_libs.append(
                    "winspool"
            ...
            if is_apple_os(self):
                ...
                if self.options.gui and self.options.widgets:
                    ...
                    self.cpp_info.components["qtPrintSupport"].frameworks.append(
                        "ApplicationServices"
                    )
                if self.settings.os == "Macos":
                ...
                if self.settings.os not in ["iOS", "tvOS"]:
                    self.cpp_info.components["qtNetwork"].frameworks.append(
                        "CoreServices"
                    )
                    self.cpp_info.components["qtNetwork"].frameworks.append(
                        "SystemConfiguration"
        ...
        for m in os.listdir(os.path.join("lib", "cmake")):
            ...
            if component_name in self.cpp_info.components:
                ...
                for helper_modules in glob.glob(
                    os.path.join(
                        self.package_folder, "lib", "cmake", m, "QtPublic*Helpers.cmake"
                    )
                ):
                ...
                component_name.endswith("Tools")
                and component_name[:-5] in self.cpp_info.components
            ):
                module = os.path.join("lib", "cmake", f"{m}", f"{m[:-5]}Macros.cmake")
                if os.path.isfile(module):
                    _add_build_module(component_name[:-5], module)
                self.cpp_info.components[component_name[:-5]].builddirs.append(
                    os.path.join("lib", "cmake", m)
                )
        ...
        for object_dir in objects_dirs:
            for m in os.listdir(object_dir):
                ...
                for root, _, files in os.walk(os.path.join(object_dir, m)):
                    ...
                    self.cpp_info.components[component].sharedlinkflags.extend(
                        obj_files
                    )
