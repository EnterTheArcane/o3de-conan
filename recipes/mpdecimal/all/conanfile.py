...
class MpdecimalConan(ConanFile):
    ...
    def _build_msvc(self):
        ...
        if self.options.cxx:
            if self.options.shared:
                copy(
                    self,
                    f"libmpdec++-{self.version}.dll",
                    libmpdecpp_folder,
                    dist_folder,
                )
                copy(
                    self,
                    f"libmpdec++-{self.version}.dll.lib",
                    libmpdecpp_folder,
                    dist_folder,
                )
            else:
        ...
        if is_msvc(self):
            ...
            if self.options.cxx:
                copy(
                    self,
                    "*",
                    self.source_path / "libmpdec++",
                    self.build_path / "libmpdec++",
                )
                with chdir(self, "libmpdec++"):
    ...
    def package_info(self):
        ...
        self.cpp_info.components["libmpdecimal"].libs = [
            "{}mpdec{}".format(*lib_pre_suf)
        ]
