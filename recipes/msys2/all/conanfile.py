...
class lock:
    def __init__(self):
        self.handle = ctypes.windll.kernel32.CreateMutexA(
            None, 0, "Global\\ConanMSYS2".encode()
        )
        if not self.handle:
...
class Recipe(ConanFile):
    ...
    def _update_pacman(self):
        with chdir(self, os.path.join(self._msys_dir, "usr", "bin")):
            try:
                ...
                self.run(
                    'bash -l -c "pacman --debug --noconfirm --ask 20 -Syuu"'
    ...
    def _kill_pacman(self):
        ...
        if self.settings.os == "Windows":
            taskkill_exe = os.path.join(
                os.environ.get("SystemRoot"), "system32", "taskkill.exe"
            )
                ...
                for taskkill_cmd in taskkill_cmds:
                    try:
                        proc = subprocess.Popen(
                            taskkill_cmd, stdout=out, stderr=err, bufsize=1
                        )
                        proc.wait()
                    except OSError as e:
    ...
    def _do_build(self):
        ...
        replace_in_file(
            self,
            os.path.join(self._msys_dir, "etc", "profile"),
            'PKG_CONFIG_PATH="',
    ...
    def package_info(self):
        ...
        self.conf_info.define(
            "tools.microsoft.bash:path", os.path.join(msys_bin, "bash.exe")
        )

        if (
            self.settings_target is not None
            and self.settings_target.os == "Windows"
            and self.settings_target.arch == "armv8"
        ):
