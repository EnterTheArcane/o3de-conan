...
class GnConan(ConanFile):
    ...
    def source(self):
        ...
        replace_in_file(
            self, os.path.join(self.source_folder, "build/gen.py"), "'clang++'", "'c++'"
        )

        # support windows arm64
        replace_in_file(
            self,
            os.path.join(self.source_folder, "src", "util", "build_config.h"),
            "defined(__aarch64__)",
    ...
    def generate(self):
        ...
        save(
            self,
            os.path.join(self.source_folder, "configure_args"),
            " ".join(configure_args),
        )
    ...
    def build(self):
        with chdir(self, self.source_folder):
            ...
            save(
                self,
                os.path.join(self.source_folder, "src", "gn", "last_commit_position.h"),
                textwrap.dedent(
                    """\
                    #pragma once
