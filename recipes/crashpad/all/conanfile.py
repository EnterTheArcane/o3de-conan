...
class Recipe(ConanFile):
    ...
    def validate(self):
        ...
        if min_compiler_version:
            if Version(self.settings.compiler.version) < min_compiler_version:
                raise ConanInvalidConfiguration(
                    "crashpad needs a c++14 capable compiler, version >= {}".format(
        ...
        get(
            ...
            **self.conan_data["sources"][self.version]["mini_chromium"],
            destination=os.path.join(
                self.source_folder, "third_party", "mini_chromium", "mini_chromium"
            ),
            strip_root=True
        )
        ...
        with chdir(self, self.source_folder):
            ...
            self.run(
                "ninja -C out/Default {targets} -j{parallel}".format(
                    targets=" ".join(targets), parallel=os.cpu_count()
                )
            )
    ...
    def package_info(self):
        ...
        self.cpp_info.components["mini_chromium_base"].builddirs = [
        ...
        if self.settings.os in ("Linux", "FreeBSD"):
            ...
            self.cpp_info.components["util"].requires.append(
        ...
        if self.settings.os == "Macos":
            self.cpp_info.components["util"].frameworks.extend(
        ...
        self.cpp_info.components["client_common"].requires = [
            "util",
            "mini_chromium_base",
        ]
        ...
        self.cpp_info.components["format"].requires = [
            "snapshot",
            "mini_chromium_base",
            "util",
        ]
        ...
        self.cpp_info.components["minidump"].requires = [
            "snapshot",
            "mini_chromium_base",
            "util",
        ]
