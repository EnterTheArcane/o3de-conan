...
class OpenImageIOConan(ConanFile):
    ...
    def validate(self):
        ...
        if (
            self.options.get_safe("with_openjph", False)
            and not self.options.with_openjpeg
        ):
            raise ConanInvalidConfiguration(
    ...
    def generate(self):
        ...
        tc.variables["USE_OPENCOLORIO"] = self.options.get_safe(
            "with_opencolorio", True
        )
        ...
        if self.options.with_ffmpeg:
            ...
            tc.cache_variables["FFMPEG_VERSION"] = (
                f'"{str(self.dependencies["ffmpeg"].ref.version)}"'
            )
        ...
        tc.cache_variables["ROBINMAP_INCLUDES"] = (
            self.dependencies["tsl-robin-map"]
            .cpp_info.includedirs[0]
            .replace("\\", "/")
        )
        tc.cache_variables["IMATH_INCLUDES"] = (
            self.dependencies["imath"].cpp_info.includedirs[0].replace("\\", "/")
        )
        tc.cache_variables["OPENEXR_INCLUDES"] = (
            self.dependencies["openexr"].cpp_info.includedirs[0].replace("\\", "/")
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_PNG"] = self.options.with_libpng
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_Freetype"] = (
            self.options.with_freetype
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_OpenColorIO"] = (
            self.options.get_safe("with_opencolorio", True)
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_OpenCV"] = (
            self.options.with_opencv
        )
        ...
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_Libheif"] = (
            self.options.with_libheif
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_LibRaw"] = self.options.with_raw
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_OpenJPEG"] = (
            self.options.with_openjpeg
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_openjph"] = (
            self.options.get_safe("with_openjph", False)
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_Ptex"] = self.options.with_ptex
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_WebP"] = (
            self.options.with_libwebp
        )
        tc.cache_variables["CMAKE_REQUIRE_FIND_PACKAGE_JXL"] = self.options.get_safe(
            "with_libjxl", False
        )

        tc.cache_variables["CMAKE_DISABLE_FIND_PACKAGE_libjpeg-turbo"] = (
        ...
        tc.cache_variables["CMAKE_DISABLE_FIND_PACKAGE_JXL"] = (
            not self.options.get_safe("with_libjxl", False)
        )
        ...
        deps.set_property(
            "tsl-robin-map", "cmake_additional_variables_prefixes", ["ROBINMAP"]
        )
        if Version(self.version) >= "3.0":
