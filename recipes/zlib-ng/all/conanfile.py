...
class ZlibNgConan(ConanFile):
    ...
    def _get_zlib_header_version(self):
        ...
        match = re.search(
            r'#define\s+ZLIB_VERSION\s+"([0-9]+\.[0-9]+\.[0-9]+)\.zlib-ng"', zlib_h
        )
    ...
    def build(self):
        ...
        if header_version and header_version != self._zlib_compat_version:
            raise ConanException(
                f"the zlib compatibility version ({header_version}) is not correctly recorded in the recipe for this zlib-ng version ({self.version})"
            )
    ...
    def package_info(self):
        ...
        if self.options.zlib_compat:
            ...
            self.cpp_info.set_property(
                "system_package_version", self._zlib_compat_version
            )
