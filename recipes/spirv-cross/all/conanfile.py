...
class Recipe(ConanFile):
    ...
    def validate(self):
        if not self.options.glsl and (
            ...
            or self.options.msl
            or self.options.cpp
            or self.options.reflect
        ):
            raise ConanInvalidConfiguration(
                "hlsl, msl, cpp and reflect require glsl enabled"
            )

        if self.options.build_executable and not (
            self.options.glsl
            and self.options.hlsl
            and self.options.msl
            and self.options.cpp
            and self.options.reflect
            and self.options.get_safe("util", True)
        ):
            raise ConanInvalidConfiguration(
                "executable can't be built without glsl, hlsl, msl, cpp, reflect and util"
            )
    ...
    def generate(self):
        ...
        tc.variables["SPIRV_CROSS_ENABLE_UTIL"] = (
            self.options.get_safe("util", False) or self.options.build_executable
        )
    ...
    def package_info(self):
        ...
        def _register_component(target_lib, requires):
            self.cpp_info.components[target_lib].set_property(
                "cmake_target_name", target_lib
            )
            if self.options.shared:
                self.cpp_info.components[target_lib].set_property(
                    "pkg_config_name", target_lib
                )
            prefix = (
                "d"
            ...
            # TODO: to remove in conan v2 once legacy generators removed
            self.cpp_info.components[target_lib].names[
                "cmake_find_package"
            ] = target_lib
            self.cpp_info.components[target_lib].names[
            ...
            self.cpp_info.components[target_lib].build_modules[
                "cmake_find_package_multi"
            ] = [self._module_file_rel_path]
