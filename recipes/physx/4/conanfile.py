...
class Recipe(ConanFile):
    ...
    def validate(self):
        ...
        if self.settings.os == "Macos":
            if self.settings.arch not in ["x86", "x86_64"]:
                raise ConanInvalidConfiguration(
                    f"{self.name} only supports x86 and x86_64 on macOS"
                )

            if valid_min_cppstd(self, 17):
                raise ConanInvalidConfiguration(
                    f"{self.name} is not supported with C++ 17. Contributions are welcome."
                )
        ...
        if self.settings.os == "Windows" and not is_msvc(self):
            raise ConanInvalidConfiguration(
                f"{self.name} only supports Visual Studio on Windows"
            )
    ...
    def _copy_external_bin(self):
        ...
        if self.settings.os == "Linux" and self.settings.arch == "x86_64":
            ...
            physx_arch = {"x86": "x86_32", "x86_64": "x86_64"}.get(
                str(self.settings.arch)
            )
            dll_info_list = [
                {
                    "pattern": "PhysXGpu*.dll",
                    "vc_ver": {"x86": "vc120", "x86_64": "vc140"}.get(
                        str(self.settings.arch)
                    ),
                },
                {
