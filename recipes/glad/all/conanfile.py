...
class Recipe(ConanFile):
    ...
    def validate(self):
        if (
            self.options.gles1_version != "None" or self.options.gles2_version != "None"
        ) and self.options.egl_version == "None":
            raise ConanInvalidConfiguration(
                f"{self.ref} Generating an OpenGLES spec requires a valid version of EGL"
            )
        if self.options.debug_layer and self.options.multicontext:
            raise ConanInvalidConfiguration(
                "The multicontext and debug layer options are incompatible"
            )
    ...
    def generate(self):
        ...
        if self.options.extensions:
            tc.cache_variables["GLAD_CONAN_EXTENSIONS"] = ";".join(
                str(self.options.extensions).split(",")
            )
    ...
    def _get_api(self):
        ...
        return ";".join(
            (
                f"{name}:{self.options.gl_profile}={version}"
                if name == "gl"
                else f"{name}={version}"
            )
