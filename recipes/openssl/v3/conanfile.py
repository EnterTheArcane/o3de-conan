...
class Recipe(ConanFile):
    ...
    def _target(self):
        ...
        if self._use_nmake:
            target = (
                f"VC-{target}"  # VC- prefix is important as it's checked by Configure
            )
        if self._is_mingw:
    ...
    def _configure_args(self):
        ...
        if self.settings.os == "Android":
            args.append(
                f" -D__ANDROID_API__={str(self.settings.os.api_level)}"
            )  # see NOTES.ANDROID
        if self.settings.os == "Windows":
        ...
        args.append(
            "no-fips" if self.options.get_safe("no_fips", True) else "enable-fips"
        )
        args.append("no-md2" if self.options.get_safe("no_md2", True) else "enable-md2")
        if str(self.options.tls_security_level) != "None":
            args.append(
                f"-DOPENSSL_TLS_SECURITY_LEVEL={self.options.tls_security_level}"
            )
        ...
        if not self.options.no_zlib:
            ...
            if self._use_nmake:
                ...
                lib_path = self._adjust_path(
                    os.path.join(
                        zlib_cpp_info.libdirs[0], f"{zlib_cpp_info.libs[0]}.lib"
                    )
                )
    ...
    def generate(self):
        ...
        if self.settings.compiler == "apple-clang":
            ...
            env.define_path(
                "CROSS_TOP", os.path.dirname(os.path.dirname(xcrun.sdk_path))
            )
    ...
    def _create_targets(self, cflags, cxxflags, defines, ldflags):
        ...
        if self.settings.os == "Neutrino":
            if self.options.shared:
                shared_extension = (
                    r'shared_extension => ".so.\$(SHLIB_VERSION_NUMBER)",'
                )
        ...
        save(
            self,
            os.path.join(self.source_folder, "Configurations", "20-conan.conf"),
            config,
        )
    ...
    def _make(self):
        with chdir(self, self.source_folder):
            ...
            if self._use_nmake:
                ...
                replace_in_file(
                    self, "Makefile", "INSTALLTOP_dir=\\", "INSTALLTOP_dir=\\\\"
                )
                if Version(self.version) >= "3.3.0":
                    # replace backslashes in paths with forward slashes
                    mkinstallvars_pl = os.path.join(
                        self.source_folder, "util", "mkinstallvars.pl"
                    )
    ...
    def build(self):
        self._make()
        configdata_pm = self._adjust_path(
            os.path.join(self.source_folder, "configdata.pm")
        )
        self.run(f"{self._perl} {configdata_pm} --dump")

    @property
    def _make_program(self):
        use_jom = self._use_nmake and self.conf.get(
            "user.openssl:windows_use_jom", False
        )
        if self._use_nmake:
    ...
    def _module_file_rel_path(self):
        return os.path.join(
            self._module_subfolder, f"conan-official-{self.name}-variables.cmake"
        )
