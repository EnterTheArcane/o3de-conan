...
class OpenSSLConan(ConanFile):
    ...
    def validate(self):
        if self.settings.os == "Emscripten":
            if not all(
                (
                    self.options.no_asm,
                    self.options.no_threads,
                    self.options.no_stdio,
                    self.options.no_tests,
                )
            ):
                raise ConanInvalidConfiguration(
                    "os=Emscripten requires openssl:{no_asm,no_threads,no_stdio,no_tests}=True"
                )
    ...
    def _target(self):
        ...
        if self._use_nmake:
            target = (
                "VC-" + target
            )  # VC- prefix is important as it's checked by Configure
        if self._is_mingw:
            target = "mingw-" + target
        return target
...
"""
        gen_info = json.loads(
            load(self, os.path.join(self.generators_folder, "gen_info.conf"))
        )
        ...
        self.output.info(
            "using target: %s -> %s" % (self._target, self._ancestor_target)
        )
    ...
    def _replace_runtime_in_file(self, filename):
        ...
        for e in ["MDd", "MTd", "MD", "MT"]:
            replace_in_file(
                self, filename, "/{} ".format(e), "/{} ".format(runtime), strict=False
            )
            replace_in_file(
                self, filename, '/{}"'.format(e), '/{}"'.format(runtime), strict=False
            )
