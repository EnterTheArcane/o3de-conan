...
class GetTextConan(ConanFile):
    ...
    def generate(self):
            ...
            def programs():
                ...
                if self._is_clang_cl:
                    return (
                        ...
                        os.environ.get("AR", "llvm-lib"),
                        os.environ.get("LD", "lld-link"),
                        rc,
                    )
                if is_msvc(self):
        ...
        if is_msvc(self) or self._is_clang_cl:
            ...
            for dependency in self.dependencies.values():
                ...
                linkflags.extend(
                    deps_cpp_info.sharedlinkflags + deps_cpp_info.exelinkflags
                )
            ...
            env.append(
                "_LINK_",
                [lib if lib.endswith(".lib") else f"{lib}.lib" for lib in libs],
            )
            env.append(
                "LDFLAGS", [f"-L{unix_path(self, p)}" for p in libdirs] + linkflags
            )
    ...
    def package(self):
        ...
        copy(
            self, "*gnuintl*.dll", self.build_folder, dest_runtime_dir, keep_path=False
        )
        ...
        copy(
            self, "*libgnuintl.h", self.build_folder, dest_include_dir, keep_path=False
        )
        rename(
            self,
