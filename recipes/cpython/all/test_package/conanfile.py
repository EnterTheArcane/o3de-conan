...
class TestPackage(ConanFile):
    ...
    def _python(self):
        if conan2:
            return self.dependencies["cpython"].conf_info.get(
                "user.cpython:python", check_type=str
            )
        else:
    ...
    def _test_module(self, module, should_work):
        try:
            self.run(
                f'{self._python} "{self.source_folder}/test_package.py" -b "{self.build_folder}" -t {module}',
                env="conanrun",
            )
        except ConanException:
            if should_work:
                self.output.warning(
                    f"Module '{module}' does not work, but should have worked"
                )
                raise
    ...
    def test(self):
        if can_run(self):
            ...
            buffer = StringIO()
            self.run(
                f"{self._python} -c \"import sys; print('.'.join(str(s) for s in sys.version_info[:3]))\"",
                buffer,
                env="conanrun",
            )
            ...
            if is_apple_os(self) and not self._cpython_option("shared"):
                ...
                if self._supports_modules:
                    os.environ["PYTHONPATH"] = os.path.join(
                        self.build_folder, self.cpp.build.libdirs[0]
                    )
                    ...
                    if self._test_setuptools:
                        os.environ["PYTHONPATH"] = os.path.join(
                            self.build_folder, "lib_setuptools"
                        )
                        self.output.info(
            ...
            if conan2:
                os.environ["PYTHONHOME"] = self.dependencies["cpython"].conf_info.get(
                    "user.cpython:pythonhome", check_type=str
                )
            else:
