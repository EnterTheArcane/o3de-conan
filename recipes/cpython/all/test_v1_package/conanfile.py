...
class Recipe(ConanFile):
    ...
    def _test_setuptools(self):
        ...
        return (
            not tools.cross_building(self, skip_x64_x86=True)
            and self._supports_modules
            and self._py_version < tools.Version("3.12")
        )
    ...
    def _supports_modules(self):
        return (
            self.settings.compiler != "Visual Studio" or self.options["cpython"].shared
        )
    ...
    def build(self):
        ...
        cmake.definitions["Python3_USE_STATIC_LIBS"] = not self.options[
            "cpython"
        ].shared
        ...
        if self._test_setuptools:
            with (
                ...
                with tools.environment_append(env):
                    setup_args = [
                        os.path.join(
                            self.source_folder, "..", "test_package", "setup.py"
                        ),
                        "build",
                        "--build-base",
                        self.build_folder,
    ...
    def _test_module(self, module, should_work):
        try:
            self.run(
                '{} "{}/../test_package/test_package.py" -b "{}" -t {} '.format(
                    self.deps_user_info["cpython"].python,
                    self.source_folder,
                    self.build_folder,
                    module,
        ...
        if should_work == works:
            ...
            if works:
                raise ConanException(
                    "Module '{}' works, but should not have worked".format(module)
                )
            else:
                self.output.warn(
                    "Module '{}' does not work, but should have worked".format(module)
                )
                raise exception
    ...
    def test(self):
        if not tools.cross_building(self, skip_x64_x86=True):
            ...
            if (
                ...
                and not self.options["cpython"].shared
            ):
                self.output.info(
                ...
                if self._supports_modules:
                    with tools.environment_append(
                        ...
                        self.output.info(
                            "Testing module (spam) using cmake built module"
                        )
                    ...
                    if self._test_setuptools:
                        with tools.environment_append(
                            ...
                            self.output.info(
                                "Testing module (spam) using setup.py built module"
                            )
                            self._test_module("spam", True)
