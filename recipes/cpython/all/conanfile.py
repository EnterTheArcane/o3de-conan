...
class CPythonConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.settings.os != "Windows":
            ...
            self.requires(
                "libxcrypt/4.4.36",
                transitive_headers=transitive_crypt,
                transitive_libs=transitive_crypt,
            )
    ...
    def validate(self):
        ...
        if is_msvc(self):
            ...
            if self.settings.build_type == "Debug" and "d" not in msvc_runtime_flag(
                self
            ):
                raise ConanInvalidConfiguration(
            ...
            if str(self.settings.arch) not in self._msvc_archs:
                raise ConanInvalidConfiguration(
                    "Visual Studio does not support this architecture"
                )
        ...
        if (
            self.options.get_safe("with_curses", False)
            and not self.dependencies["ncurses"].options.with_widec
        ):
            raise ConanInvalidConfiguration(
                "cpython requires ncurses with wide character support"
            )
        ...
        if (
            ...
            and Version(self.version) >= "3.12"
        ):
            raise ConanInvalidConfiguration(
                "FIXME: GCC 9 produces an internal compiler error locally, and a link error in CCI"
            )
    ...
    def _generate_autotools(self):
        ...
        if self.options.with_tkinter and Version(self.version) < "3.11":
            ...
            if (
                ...
                and not self.dependencies["tk"].options.shared
            ):
    ...
    def _inject_conan_props_file(self, project_basename, dep_name, condition=True):
        if condition:
            ...
            replace_in_file(
                ...
                search,
                search
                + f'<Import Project="{self.generators_folder}/conan_{dep_name}.props" />',
            )
    ...
    def _patch_setup_py(self):
        ...
        if self.options.get_safe("with_curses", False):
            ...
            libs = (
                libcurses.libs + libcurses.system_libs + tinfo.libs + tinfo.system_libs
            )
            replace_in_file(
                self,
                setup_py,
    ...
    def _patch_msvc_projects(self):
        ...
        self._regex_replace_in_file(
            self._msvc_project_path("_bz2"), r".*Include=\"\$\(bz2Dir\).*", ""
        )
        ...
        if self._supports_modules:
            ...
            replace_in_file(
                self,
                self._msvc_project_path("_ctypes"),
            ...
            if Version(self.version) < "3.11":
                ...
                replace_in_file(
                    self, self._msvc_project_path("_ctypes"), "FFI_BUILDING;", ""
                )

        # Don't import vendored openssl
        replace_in_file(
            self,
            self._msvc_project_path("_hashlib"),
            '<Import Project="openssl.props" />',
            "",
        )
        replace_in_file(
            self,
            self._msvc_project_path("_ssl"),
            '<Import Project="openssl.props" />',
            "",
        )
        ...
        self._regex_replace_in_file(
            self._msvc_project_path("_decimal"),
            r".*Include=\"\.\.\\Modules\\_decimal\\.*\.h.*",
            "",
        )
        self._regex_replace_in_file(
            self._msvc_project_path("_decimal"),
            r".*Include=\"\.\.\\Modules\\_decimal\\libmpdec\\.*\.c.*",
            "",
        )
        ...
        replace_in_file(
            self,
            self._msvc_project_path("_sqlite3"),
        ...
        replace_in_file(
            self,
            self._msvc_project_path("_lzma"),
            "<AdditionalDependencies>$(OutDir)liblzma$(PyDebugExt).lib;",
        ...
        replace_in_file(
            self,
            self._msvc_project_path("pyexpat"),
        ...
        replace_in_file(
            self,
            self._msvc_project_path("pyexpat"),
            ("HAVE_EXPAT_H;" if Version(self.version) < "3.11" else "") + "XML_STATIC;",
            "",
        )
        self._regex_replace_in_file(
            self._msvc_project_path("pyexpat"),
            r'.*Include=\"\.\.\\Modules\\expat\\.*" />',
            "",
        )
        ...
        replace_in_file(
            self,
            self._msvc_project_path("_elementtree"),
            r"<AdditionalIncludeDirectories>..\Modules\expat;",
            "<AdditionalIncludeDirectories>",
        )
        # Remove XML_STATIC, this should conditionally be set by the expat library.
        replace_in_file(
            self, self._msvc_project_path("_elementtree"), "XML_STATIC;", ""
        )
        ...
        self._regex_replace_in_file(
            self._msvc_project_path("_tkinter"), r".*Include=\"\$\(tcltkdir\).*", ""
        )

        # Disable "ValidateUcrtbase" target (TODO: Why?)
        replace_in_file(
            self,
            self._msvc_project_path("python"),
        ...
        if Version(self.version) < "3.11":
            ...
            replace_in_file(
                self,
                self._msvc_project_path("_freeze_importlib"),
        ...
        self._inject_conan_props_file(
            "_bz2", "bzip2", self.options.get_safe("with_bz2")
        )
        ...
        self._inject_conan_props_file(
            "_sqlite3", "sqlite3", self.options.get_safe("with_sqlite3")
        )
        self._inject_conan_props_file(
            "_tkinter", "tk", self.options.get_safe("with_tkinter")
        )
        ...
        self._inject_conan_props_file(
            "_lzma", "xz_utils", self.options.get_safe("with_lzma")
        )
        self._inject_conan_props_file(
            "_bsddb", "libdb", self.options.get_safe("with_bsddb")
        )
    ...
    def _patch_sources(self):
        ...
        if Version(self.version) < "3.12":
            replace_in_file(
                self,
                os.path.join(self.source_folder, "Makefile.pre.in"),
        ...
        conantoolchain_props = os.path.join(
    ...
    def _solution_projects(self):
        if self.options.shared:
            ...
            projects = set(
                m.group(1)
                for m in re.finditer('"([^"]+)\\.vcxproj"', open(solution_path).read())
            )
    ...
    def _msvc_artifacts_path(self):
        ...
        return os.path.join(
            self.source_folder, "PCbuild", build_subdir_lut[str(self.settings.arch)]
        )
    ...
    def _msvc_package_copy(self):
        ...
        copy(
            ...
            dst=os.path.join(
                self.package_folder, self._msvc_install_subprefix, "include"
            ),
        )
        copy(
            self,
        ...
        rmdir(
            self,
            os.path.join(
                self.package_folder, self._msvc_install_subprefix, "Lib", "test"
            ),
        )
        ...
        for fname in packages.values():
            unzip(
                ...
                destination=os.path.join(
                    self.package_folder, "bin", "Lib", "site-packages"
                ),
            )

        interpreter_path = os.path.join(build_path, self._cpython_interpreter_name)
        lib_dir_path = os.path.join(
            self.package_folder, self._msvc_install_subprefix, "Lib"
        ).replace("\\", "/")
        self.run(
            f"{interpreter_path} -c \"import compileall; compileall.compile_dir('{lib_dir_path}')\""
        )
    ...
    def _write_cmake_findpython_wrapper_file(self):
        ...
        if is_msvc(self):
            python_exe = (
                "${CMAKE_CURRENT_LIST_DIR}/../../" + self._cpython_interpreter_name
            )
            python_library = "${CMAKE_CURRENT_LIST_DIR}/../" + self._exact_lib_name
        else:
            python_exe = (
                "${CMAKE_CURRENT_LIST_DIR}/../../bin/" + self._cpython_interpreter_name
            )
            python_library = "${CMAKE_CURRENT_LIST_DIR}/../" + self._exact_lib_name

        cmake_file = os.path.join(
            self.package_folder, self._cmake_module_path, "use_conan_python.cmake"
        )
        content = template.replace("@PYTHON_EXECUTABLE@", python_exe).replace(
    ...
    def package_info(self):
        ...
        self.cpp_info.set_property(
            "cmake_build_modules",
            [os.path.join(self._cmake_module_path, "use_conan_python.cmake")],
        )
        ...
        if self._supports_modules:
            ...
            if self.settings.os != "Windows":
                if not is_apple_os(self):
                    self.cpp_info.components["_hidden"].requires.append(
        ...
        self.conf_info.define(
            "user.cpython:module_requires_pythonhome", pythonhome_required
        )
        ...
        if is_msvc(self):
            if self.options.env_vars:
                ...
                # TODO remove once Conan 1.x is no longer supported
                self.output.info(
                    f"Setting PYTHONHOME environment variable: {pythonhome}"
                )
