...
class Recipe(ConanFile):
    ...
    def requirements(self):
        ...
        if Version(conan_version).major < "2":
            ...
            self.requires(
                self._require("spirv-tools"),
                private=not hasattr(self, "settings_build"),
            )
        else:
            self.requires(self._require("spirv-tools"))
        self.requires(self._require("vulkan-headers"), transitive_headers=True)
        if self.options.get_safe("with_wsi_xcb") or self.options.get_safe(
            "with_wsi_xlib"
        ):
    ...
    def _require(self, recipe_name):
        if recipe_name not in self._dependencies_versions:
            raise ConanException(
                f"{recipe_name} is missing in {self._dependencies_filename}"
            )
    ...
    def build_requirements(self):
        if self._needs_pkg_config and not self.conf.get(
    ...
    def generate(self):
        ...
        if self.settings.os in ["Linux", "FreeBSD"]:
            tc.variables["BUILD_WSI_XCB_SUPPORT"] = self.options.get_safe(
                "with_wsi_xcb"
            )
            tc.variables["BUILD_WSI_XLIB_SUPPORT"] = self.options.get_safe(
                "with_wsi_xlib"
            )
            tc.variables["BUILD_WSI_WAYLAND_SUPPORT"] = self.options.get_safe(
                "with_wsi_wayland"
            )
    ...
    def _patch_sources(self):
        ...
        if Version(self.version) >= "1.3.235" and Version(self.version) < "1.3.239":
            ...
            sanitized_vk_version = (
                f"{vk_version.major}.{vk_version.minor}.{vk_version.patch}"
            )
            replace_in_file(
                self,
        ...
        if self.settings.os == "Android":
            ...
            if Version(self.version) < "1.3.239":
                ...
                cmake_path = os.path.join(
                    self.source_folder, "layers", "CMakeLists.txt"
                )
        ...
        if self.settings.os == "Windows":
            ...
            for ext in ("*.dll", "*.json"):
                for bin_file in glob.glob(os.path.join(lib_dir, ext)):
                    shutil.move(
                        bin_file, os.path.join(bin_dir, os.path.basename(bin_file))
                    )
        else:
